...
#define CONFIG_ESP_QCLOUD_SNTP_SERVER_NAME "pool.ntp.org"

static bool g_init_done = false;
static const char *TAG = "esp_qcloud_timesync";
...
esp_err_t esp_qcloud_timesync_start()
{
    ...
    char *sntp_server_name = CONFIG_ESP_QCLOUD_SNTP_SERVER_NAME;
...
esp_err_t esp_qcloud_timesync_wait(TickType_t ticks_to_wait)
{
    ...
    while (ticks_remaining > 0) {
        if (esp_qcloud_timesync_check() == true) {
            break;
        }
