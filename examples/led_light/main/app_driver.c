...
static uint16_t g_value = DEFAULT_VALUE;
...
esp_err_t app_light_set_value(uint16_t brightness)
{
    g_value = brightness;
...
uint16_t app_light_get_value()
{
    return g_value;
}
uint16_t app_light_get_hue()
{
    return g_hue;
}
uint16_t app_light_get_saturation()
{
    return g_saturation;
}
