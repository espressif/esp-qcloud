// Copyright 2017 Espressif Systems (Shanghai) PTE LTD
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at

//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#include <string.h>
#include <sys/param.h>

#include "esp_wifi.h"
#include "esp_console.h"
#include "esp_partition.h"

#include "esp_qcloud_iothub.h"
#include "esp_qcloud_log.h"
#include "esp_qcloud_storage.h"


#define MDEBUG_FLASH_FILE_MAX_NUM   2           /**< Create several files */
#define MDEBUG_FLASH_FILE_MAX_SIZE  CONFIG_QCLOUD_LOG_FILE_MAX_SIZE   /**< File storage size */
#define MDEBUG_FLASH_STORE_KEY      "log_info"

/**
 * @brief Create files configuration
 */
typedef struct {
    size_t size; /**< Files size */
    int offset;  /**< Set file pointer offset */
    uint32_t addr;
} flash_log_info_t;

static int g_log_index                         = 0;
static flash_log_info_t *g_log_info            = NULL;
static bool g_esp_qcloud_log_flash_init_flag = false;
static const esp_partition_t *g_log_part       = NULL;
static const char *TAG                         = "esp_qcloud_log_flash";

esp_err_t esp_qcloud_log_flash_init()
{
    if (g_esp_qcloud_log_flash_init_flag) {
        return ESP_OK;
    }

    esp_err_t ret = ESP_OK;

    esp_partition_iterator_t part_itra = esp_partition_find(ESP_PARTITION_TYPE_DATA,
                                         ESP_PARTITION_SUBTYPE_ANY, CONFIG_QCLOUD_LOG_PARTITION_LABEL);
    ESP_QCLOUD_ERROR_CHECK(!part_itra, ESP_ERR_NOT_SUPPORTED, "partition no find, subtype: 0x%x, label: %s",
                           ESP_PARTITION_SUBTYPE_ANY, CONFIG_QCLOUD_LOG_PARTITION_LABEL);

    g_log_part = esp_partition_get(part_itra);

    ESP_QCLOUD_ERROR_CHECK(!g_log_part, ESP_ERR_NOT_SUPPORTED, "esp_partition_get");
    ESP_QCLOUD_ERROR_CHECK(g_log_part->size < MDEBUG_FLASH_FILE_MAX_SIZE, ESP_ERR_NOT_SUPPORTED,
                           "Log file (%d Byte) size must be smaller than partition size (%d Byte).",
                           MDEBUG_FLASH_FILE_MAX_SIZE, g_log_part->size);
    ESP_QCLOUD_ERROR_CHECK(MDEBUG_FLASH_FILE_MAX_SIZE / MDEBUG_FLASH_FILE_MAX_NUM % 4096 != 0, ESP_ERR_NOT_SUPPORTED,
                           "The size of the log partition must be an integer of %d KB.", MDEBUG_FLASH_FILE_MAX_NUM * 4);

    g_log_info = esp_qcloud_storage_get(MDEBUG_FLASH_STORE_KEY);

    if (!g_log_info) {
        g_log_info = ESP_QCLOUD_CALLOC(MDEBUG_FLASH_FILE_MAX_NUM, sizeof(flash_log_info_t));
        ret = esp_partition_erase_range(g_log_part, CONFIG_QCLOUD_LOG_PARTITION_OFFSET, MDEBUG_FLASH_FILE_MAX_SIZE);
        ESP_QCLOUD_ERROR_CHECK(ret != ESP_OK, ret, "esp_partition_erase_range");
    }

    /**< Create two files */
    for (size_t i = 0, min_size = MDEBUG_FLASH_FILE_MAX_SIZE / MDEBUG_FLASH_FILE_MAX_NUM; i < MDEBUG_FLASH_FILE_MAX_NUM; i++) {
        (g_log_info + i)->addr = CONFIG_QCLOUD_LOG_PARTITION_OFFSET + MDEBUG_FLASH_FILE_MAX_SIZE / MDEBUG_FLASH_FILE_MAX_NUM * i;

        /**< Whether file is full */
        if ((g_log_info + i)->size < MDEBUG_FLASH_FILE_MAX_SIZE / MDEBUG_FLASH_FILE_MAX_NUM && (g_log_info + i)->size < min_size) {
            min_size = (g_log_info + i)->size;
            g_log_index  = i;  /**< File index */
        }
    }

    g_esp_qcloud_log_flash_init_flag = true;
    ESP_LOGI(TAG, "Mdebug flash initialized successfully");
    ESP_LOGI(TAG, "Log save partition subtype: label: %s, addr: 0x%d, offset: %d, size: %d",
             CONFIG_QCLOUD_LOG_PARTITION_LABEL, g_log_part->address, CONFIG_QCLOUD_LOG_PARTITION_OFFSET, g_log_part->size);

    return ESP_OK;
}

esp_err_t esp_qcloud_log_flash_deinit()
{
    if (!g_esp_qcloud_log_flash_init_flag) {
        return ESP_FAIL;
    }

    g_esp_qcloud_log_flash_init_flag = false;

    ESP_LOGD(TAG, "Mdebug flash de-initialized successfully");

    return ESP_OK;
}

esp_err_t esp_qcloud_log_flash_write(const char *data, size_t size, esp_log_level_t level, struct tm *log_time)
{
    esp_err_t ret              = ESP_OK;
    static bool flag_timestamp = true;

    ESP_QCLOUD_PARAM_CHECK(data);
    ESP_QCLOUD_PARAM_CHECK(size > 0);

    if (!g_esp_qcloud_log_flash_init_flag) {
        return ESP_FAIL;
    }

    /**
     * @brief Get the address of the pointer to the next file and clear the file to be written
     * and clear offset size.
     */
    if ((g_log_info + g_log_index)->size + size > MDEBUG_FLASH_FILE_MAX_SIZE / MDEBUG_FLASH_FILE_MAX_NUM) {
        g_log_index = (g_log_index + 1) % MDEBUG_FLASH_FILE_MAX_NUM;
        (g_log_info + g_log_index)->size   = 0;
        (g_log_info + g_log_index)->offset = 0;

        ret = esp_partition_erase_range(g_log_part, (g_log_info + g_log_index)->addr, MDEBUG_FLASH_FILE_MAX_SIZE / MDEBUG_FLASH_FILE_MAX_NUM);
        ESP_QCLOUD_ERROR_CHECK(ret != ESP_OK, ret, "esp_partition_erase_range, addr: %x", (g_log_info + g_log_index)->addr);
    }

    static uint32_t s_event_send_tick = 0;

    if (esp_qcloud_log_flash_size() > MDEBUG_FLASH_FILE_MAX_SIZE / MDEBUG_FLASH_FILE_MAX_NUM
            && (xTaskGetTickCount() - s_event_send_tick > 30000 || !s_event_send_tick)) {
        s_event_send_tick = xTaskGetTickCount();
        esp_event_post(QCLOUD_EVENT, QCLOUD_EVENT_DEBUG_FLASH_FULL, NULL, 0, portMAX_DELAY);
    }

    ESP_QCLOUD_LOG_PRINTF("esp_partition_write, addr: %d, offset: %d, log_index: %d, size: %d\n",
                          (g_log_info + g_log_index)->addr, (g_log_info + g_log_index)->size, g_log_index, size);

    if (flag_timestamp) {
        flag_timestamp = false;

        /**< Get the current timestamp */
        char strtime_buf[32] = {0};
        strftime(strtime_buf, sizeof(strtime_buf), "[%Y-%m-%d %H:%M:%S] ", log_time);

        /**
         * @brief Change the write file address, then wirte timestamp data
         */
        ret = esp_partition_write(g_log_part, (g_log_info + g_log_index)->addr + (g_log_info + g_log_index)->size, strtime_buf, strlen(strtime_buf));
        ESP_QCLOUD_ERROR_CHECK(ret != ESP_OK, ret, "esp_partition_write");
        (g_log_info + g_log_index)->size += strlen(strtime_buf);
    }

    /**
     * @brief Change the write file address, then wirte log data after the timestamp.
     *
     * @note First need to get the length of the timestamp, then write the log data after the timestamp
     */
    ret = esp_partition_write(g_log_part, (g_log_info + g_log_index)->addr + (g_log_info + g_log_index)->size, data, size);
    ESP_QCLOUD_ERROR_CHECK(ret != ESP_OK, ret, "esp_partition_write");
    (g_log_info + g_log_index)->size += size;

    /**
     * @brief Here in order to read the data address written by the file,Get where the data address is written.
     *
     */
    esp_qcloud_storage_set(MDEBUG_FLASH_STORE_KEY, g_log_info, sizeof(flash_log_info_t) * MDEBUG_FLASH_FILE_MAX_NUM);

    if (data[size - 1] == '\n') {
        flag_timestamp = true;
    }

    return ESP_OK;
}

esp_err_t esp_qcloud_log_flash_read(char *data, size_t *size)
{
    esp_err_t ret    = ESP_OK;
    size_t read_size = 0;
    ESP_QCLOUD_PARAM_CHECK(data);
    ESP_QCLOUD_PARAM_CHECK(size && *size > 0);

    if (!g_esp_qcloud_log_flash_init_flag) {
        *size = 0;
        return ESP_FAIL;
    }

    /**
     * @brief First find Target file index address, then read the this files data
     *
     * @note The file address read is different from the file address written, so that the same address
     * can be prevented from being written and read.
     */
    for (int i = 0; i < MDEBUG_FLASH_FILE_MAX_NUM && *size > read_size; ++i, data += read_size) {
        flash_log_info_t *log_info = g_log_info + (g_log_index + 1 + i) % MDEBUG_FLASH_FILE_MAX_NUM;
        ssize_t data_size = MIN(*size - read_size, log_info->size - log_info->offset);

        if (data_size > 0) {
            ret = esp_partition_read(g_log_part, log_info->addr + log_info->offset, data, data_size);

            if (ret != ESP_OK) {
                ESP_LOGW(TAG, "<%s> fread packet, ret: %d, *size: %d, read_size: %d, log_info->offset: %d, log_info->size: %d, index: %d",
                         strerror(errno), ret, *size, read_size, log_info->offset, log_info->size, i);
                log_info->offset = log_info->size = 0;
                break;
            }

            read_size += data_size;
            log_info->offset += data_size;
            ESP_QCLOUD_LOG_PRINTF("esp_partition_read, data_size: %d, read_size: %d, log_info->offset: %d, log_info->size: %d, ret: %d, index: %d, data: %.*s\n",
                                  data_size, read_size, log_info->offset, log_info->size, ret, i, data_size, data);

            esp_qcloud_storage_set(MDEBUG_FLASH_STORE_KEY, g_log_info, sizeof(flash_log_info_t) * MDEBUG_FLASH_FILE_MAX_NUM);
        }
    }

    *size = read_size;
    return read_size > 0 ? ESP_OK : ESP_FAIL;
}

esp_err_t esp_qcloud_log_flash_erase()
{
    if (!g_esp_qcloud_log_flash_init_flag) {
        return ESP_FAIL;
    }

    esp_err_t ret = ESP_OK;

    ret = esp_qcloud_storage_erase(MDEBUG_FLASH_STORE_KEY);
    ESP_QCLOUD_ERROR_CHECK(ret != ESP_OK, ret, "esp_qcloud_storage_erase");

    ret = esp_partition_erase_range(g_log_part, CONFIG_QCLOUD_LOG_PARTITION_OFFSET, MDEBUG_FLASH_FILE_MAX_SIZE);
    ESP_QCLOUD_ERROR_CHECK(ret != ESP_OK, ret, "esp_partition_erase_range");

    return ESP_OK;
}

size_t esp_qcloud_log_flash_size()
{
    if (!g_esp_qcloud_log_flash_init_flag) {
        return 0;
    }

    size_t size = 0;

    for (size_t i = 0; i < MDEBUG_FLASH_FILE_MAX_NUM; i++) {
        if ((g_log_info + i)->size >= (g_log_info + i)->offset) {
            size += (g_log_info + i)->size - (g_log_info + i)->offset;
        }
    }

    return size;
}
